...
#         use .children; detail rows are the next entry in the table's own
#         row list (no nextSibling). Diagnostics counters added to the
#         result object for FirewallDiagnostics.csv.
...
function Get-TableRows {
    ...
    # Rows that belong to THIS table only (not rows of tables nested inside
    # it). Uses .children, the same IHTMLElement interface as parentElement,
    # className and innerText, which the rest of this module already relies
    # on successfully. A table's children are TBODY/THEAD/TFOOT sections
    # (or TR directly); each section's children are its rows.
    $Items = [System.Collections.ArrayList]::new()

    foreach ($Child in @(Get-ElementChildren -Element $Table))
    {
        $ChildTag = Get-NodeTagName $Child
        ...
        if ($ChildTag -in @('TBODY', 'THEAD', 'TFOOT'))
        {
            foreach ($SectionChild in @(Get-ElementChildren -Element $Child))
            {
                if ((Get-NodeTagName $SectionChild) -eq 'TR')
                {
                    [void]$Items.Add($SectionChild)
                }
            }
        }
    }
...
function Get-RowCells {
    ...
    # Cells that belong to THIS row only (not cells of a table nested inside
    # one of its cells).
    $Items = [System.Collections.ArrayList]::new()

    foreach ($Child in @(Get-ElementChildren -Element $Row))
    {
        if ((Get-NodeTagName $Child) -eq $TagName)
        {
            [void]$Items.Add($Child)
        }
    }
...
function Get-ElementSourceIndex {
    ...
    # Position of the element in document order.
    if ($null -eq $Element)
    {
        return -1
    }
...
function New-HtmlParseResult {
    ...
    [PSCustomObject]@{
        ...
        Diagnostics   = [PSCustomObject]@{
            FirewallTablesFound = 0
            RuleRows            = 0
            DetailTablesFound   = 0
            FirewallTableInfo   = New-Object System.Collections.ArrayList
        }
    }
}
...
function Get-NestedDetailRows {
    ...
    # A detail row is the row directly after a setting row: exactly one cell,
    # and that cell contains a table. The caller passes the next row from the
    # table's own row list, so no sibling walking is needed.
    if ($null -eq $NextRow)
    {
        return @()
    }
...
function Parse-StandardPolicyTable {
    ...
    for ($RowIndex = 0; $RowIndex -lt $Rows.Count; $RowIndex++)
    {
        ...
        $DetailRows = @(Get-NestedDetailRows -NextRow $NextRow)

        if ($FirewallDetailMode)
        {
            $Result.Value.Diagnostics.RuleRows++
...
function Get-GPOSettingsFromHtml {
    ...
    try
    {
        ...
        # Inbound/Outbound Rules headings with their document position, used
        # to give each firewall rule table its direction.
        $FirewallHeadings = [System.Collections.ArrayList]::new()

        foreach ($Span in @(Get-AllTags -Root $Doc -TagName 'SPAN'))
        {
            if (-not (Test-ElementHasClass -Element $Span -ClassName 'sectionTitle'))
            {
                continue
            ...
            $SpanText = Get-CleanText $Span

            if ($SpanText -in @('Inbound Rules', 'Outbound Rules'))
            {
                [void]$FirewallHeadings.Add(
                    [PSCustomObject]@{
                        Index     = Get-ElementSourceIndex -Element $Span
                        Direction = ($SpanText -replace ' Rules$', '')
                    }
                )
            }
        }
        ...
        foreach ($Table in $AllTables)
        {
            try
            {
                ...
                # Firewall rule tables are recognized by their column headers
                # (Name | Description | Winning GPO). In the sample reports
                # this shape appears only under Inbound/Outbound Rules, so
                # detection does not depend on finding the section heading.
                $IsFirewallRuleTable =
                    ($Headers.Count -eq 3) -and
                    ($Headers[0] -ieq 'Name') -and
                    ($Headers[1] -ieq 'Description') -and
                    ($Headers[2] -match '(?i)^winning gpo$')

                if (-not $IsFirewallRuleTable)
                {
                    $IsFirewallRuleTable =
                        ($Headers.Count -eq 3) -and
                        ($Headers[2] -match '(?i)^winning gpo$') -and
                        (Test-IsInSection -Element $Table -SectionTitles @('Inbound Rules', 'Outbound Rules'))
                }

                if ($IsFirewallRuleTable)
                {
                    # Direction: the nearest Inbound/Outbound Rules heading
                    # before this table in document order.
                    $Direction       = ''
                    $DirectionSource = ''
                    $TableIndex      = Get-ElementSourceIndex -Element $Table

                    if ($TableIndex -ge 0)
                    {
                        $BestIndex = -1

                        foreach ($Heading in $FirewallHeadings)
                        {
                            if ($Heading.Index -lt $TableIndex -and $Heading.Index -gt $BestIndex)
                            {
                                $BestIndex       = $Heading.Index
                                $Direction       = $Heading.Direction
                                $DirectionSource = 'DocumentOrder'
                            }
                        }
                    }
                    ...
                    if ([string]::IsNullOrWhiteSpace($Direction))
                    {
                        ...
                        {
                            $Direction       = 'Outbound'
                            $DirectionSource = 'SectionHeading'
                        }
                        elseif (Test-IsInSection -Element $Table -SectionTitles @('Inbound Rules'))
                        {
                            $Direction       = 'Inbound'
                            $DirectionSource = 'SectionHeading'
                        }
                        ...
                        {
                            $Direction       = 'Unknown'
                            $DirectionSource = 'NotFound'
                        }
                    }

                    $RulesBefore = $Result.FirewallRules.Count
                    $RowsBefore  = $Result.Diagnostics.RuleRows
                    $FoundBefore = $Result.Diagnostics.DetailTablesFound

                    $Result.Diagnostics.FirewallTablesFound++
                    ...
                    [void]$Result.Diagnostics.FirewallTableInfo.Add(
                        [PSCustomObject]@{
                            ReportName        = $ReportName
                            Headers           = ($Headers -join ' | ')
                            Direction         = $Direction
                            DirectionSource   = $DirectionSource
                            TableRows         = @(Get-TableRows -Table $Table).Count
                            RuleRows          = $Result.Diagnostics.RuleRows - $RowsBefore
                            DetailTablesFound = $Result.Diagnostics.DetailTablesFound - $FoundBefore
                            RulesAdded        = $Result.FirewallRules.Count - $RulesBefore
                        }
                    )
